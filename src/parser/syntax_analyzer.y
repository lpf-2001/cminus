...
%token <node> ELSE IF INT RETURN VOID WHILE FLOAT ARRAY ADD SUB
%token <node> MUL DIV LT LTE GT GTE EQ NEQ ASSIN SEMICOLON COMMA
%token <node> LPARENTHESE RPARENTHESE LBRACKET RBRACKET LBRACE RBRACE
%token <node> IDENTIFIER INTEGER FLOATPOINT ERROR
%type <node> program type-specifier relop addop mulop declaration-list
%type <node> declaration var-declaration fun-declaration local-declarations
%type <node> compound-stmt statement-list statement expression-stmt
%type <node> iteration-stmt selection-stmt return-stmt expression
%type <node> var additive-expression term factor integer float call
%type <node> params param-list param args arg-list simple-expression
%start program
